...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadData(url, addFeatureFunc) {
        try {
            const fc = await (await fetch(`${url}?t=${Date.now()}`, { cache: 'no-store' })).json();
...
function addPinFeature(feat) {
  ...
  L.marker([lat, lng], { icon: pinIcon })
    ...
    async function savePin(lng, lat, title, description) {
        const f = {
            type: 'Feature',
            geometry: { type: 'Point', coordinates: [lng, lat] }, // store [lng, lat]
        ...
        if (!r.ok) throw new Error('Failed to save pin.');
        addPinFeature(await r.json());
    }
...
function addPhotoFeature(feat) {
  ...
  L.marker([lat, lng], { icon: photoIcon })
    ...
    ui.btnAddPin.onclick = () => {
        if (state.dropPinArmed) return disarmPinDrop();
        state.dropPinArmed = true;
        map.getContainer().style.cursor = 'crosshair';

        map.getContainer().addEventListener('click', dropPinCaptureHandler = async (ev) => {
            ...
            try {
                await savePin(latlng.lng, latlng.lat, title, description); // [lng, lat]
            } catch (e) {
    ...
    ui.btnCamShot.onclick = async () => {
        ...
        const [w, h] = [ui.video.videoWidth, ui.video.videoHeight];
        if (!w || !h) return;
        ...
        try {
            ...
            // Add the new photo to the map immediately
            const feature = {
                type: 'Feature',
