...
function pinsStore() {
  ...
  return {
    async put(key, jsonText) {
      await s.set(key, jsonText, { contentType: "application/json" });
    },
    async list() {
...
// netlify/functions/pins.mjs
var config = { path: "/api/pins" };
var JSON_HEADERS = { "Content-Type": "application/json" };
async function handler(event) {
  const pins = pinsStore();
  try {
    if (event.httpMethod === "GET") {
      const pinObjects = await pins.list();
      const features = pinObjects.filter((pin) => pin && typeof pin.lat === "number" && typeof pin.lng === "number").map((pin) => ({
        type: "Feature",
        geometry: {
          type: "Point",
